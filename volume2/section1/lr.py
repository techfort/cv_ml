...
pos_samples = [extract(get_image(POS_CLASS, x))[0:FEAT_LEN] for x in range(1, NUM_SAMPLES)]
neg_samples = [extract(get_image(NEG_CLASS, x))[0:FEAT_LEN] for x in range(1, NUM_SAMPLES)]

pos_samples = [x for x in pos_samples if len(x) == FEAT_LEN]
neg_samples = [x for x in neg_samples if len(x) == FEAT_LEN] 
...
""" testing L2 """
LR.setRegularization(cv2.ml.LOGISTIC_REGRESSION_REG_L2)
""" testing mini-batch """
